"""
test_wiener_denoise.py — AM signal demodulated with Wiener (MMSE-LSA) denoise.

Regression test: with `wiener.enabled = true`, the AM channel must still
produce an MP3 output of the expected duration. Asserts the new AUD-4 path
...
def pytest_generate_tests(metafunc):
    """Parametrize test_wiener_denoise over all available binaries."""
...
def test_wiener_denoise(
    ...
    """AM signal denoised by Wiener → MP3 must contain ≈10s of audio."""
    ...
    filename_template = "wiener_denoise"
    ...
    ), "Expected non-zero activity counter when Wiener denoise is active"
    assert (
